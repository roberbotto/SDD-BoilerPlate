---
name: opsx-apply
description: Native Codex workflow for `opsx:apply`. Implement tasks from an OpenSpec change (Experimental). Uses scope metadata for FE/BE. If scope.frontend and design-linked are true, fetch design structure live from Figma MCP before implementing frontend tasks.
---
...
- If ambiguous, run `openspec list --json` to get available changes and use the **direct Codex user prompt** to let the user select
...
Do NOT scan templates or unrelated directories (e.g., ai-specs/specs/templates/**, legacy command directories, historical archives) unless explicitly required by the current task.
...
All tasks complete! Archive with `$opsx-archive`.
```
