...
description: Native Codex workflow for `ai-specs:init-brownfield`. Adopt an existing codebase into the SDD workflow. Discovers stack, documents architecture, generates technical and functional baselines.
---
...
Codex skill: `$ai-specs-init-brownfield`
...
Use a direct Codex user prompt to confirm:
...
### Ready For
- Codex skill: `$ai-specs-new-us` → create new user stories | `$ai-specs-new-us`
- Codex skill: `$opsx-new` → create changes with delta specs | `$opsx-new`
- Codex skill: `$opsx-explore` → investigate the codebase | `$opsx-explore`

### Recommendations
- [ ] Review generated specs and correct any inaccuracies
- [ ] Add scenarios for critical business logic not captured
- [ ] Consider running `$opsx-explore` `$opsx-explore` in Codex on the most complex module
```
