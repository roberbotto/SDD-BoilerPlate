---
name: opsx-continue
description: Native Codex workflow for `opsx:continue`. Continue working on a change - create the next artifact (Experimental). Uses scope/design metadata (backend/frontend/design-linked) when present to generate deterministic tasks.
---
...
Run `openspec list --json` to get available changes sorted by most recently modified. Then use the **direct Codex user prompt** to let the user select which change to work on.
...
- Suggest: "All artifacts created! Use `$opsx-apply` or `$opsx-archive`."
- STOP
