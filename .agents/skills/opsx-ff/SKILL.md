---
name: opsx-ff
description: Native Codex workflow for `opsx:ff`. Create a change and generate all artifacts needed for implementation in one go
---
...
**If no input provided**, use the **direct Codex user prompt** (open-ended) to ask:
...
Parse the JSON to get `artifacts` and `applyRequires`. Use **Codex plan tracking** to track all artifacts.
